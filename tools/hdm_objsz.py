# -*- coding: utf-8 -*-
"""P18e: sweep with object face-size field ON + local projection."""
import json
import os
import sys
import time
...
def run(sf):
    ...
    rec = {"shell_factor": sf, "nodes": len(verts), "leaves": len(boxes),
           "x": ax["x"], "y": ax["y"],
...
def main():
    out = []
    for sf in (0.3, 0.4, 0.5, 0.25, 0.35, 0.45):
        try:
            out.append(run(sf))
        except Exception as e:
            print({"sf": sf, "error": repr(e)}, flush=True)
    out.sort(key=lambda r: r.get("score", 9e9))
    pout = os.path.join(ROOT, "tools", "probe_work", "hdm_objsz.json")
