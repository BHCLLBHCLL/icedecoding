# -*- coding: utf-8 -*-
"""P19-1: lattice-quantized surface nodes — partial overlap reproduction."""
import json
import os
import sys
import time
...
def run(pitch, zfrac, lattice, base_step=0.02):
    jdir = os.path.join(r"D:\training\icepak", JOB)
    t0 = time.time()
...
def main():
    out = []
    for pitch, zf, lat in ((0.10, 0.5, False), (0.10, 0.5, True),
                           (0.165, 1.0, True), (0.165, 0.5, True),
                           (0.12, 0.5, True), (0.10, 1.0, True)):
        try:
            out.append(run(pitch, zf, lat))
        except Exception as e:
            print({"pitch": pitch, "zfrac": zf, "lat": lat,
                   "error": repr(e)}, flush=True)
    out.sort(key=lambda r: r.get("score", 9e9))
    pout = os.path.join(ROOT, "tools", "probe_work", "hdm_lattice.json")
