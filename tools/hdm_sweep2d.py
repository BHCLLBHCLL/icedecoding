# -*- coding: utf-8 -*-
"""P18d: 2D bracket sweep (shell_factor x projection tolerance) with
curv_c fixed at 0.165 — compress the +/-7% x/y spectrum deviation."""
import json
import os
import sys
import time
...
def run(sf, tf):
    ...
    boxes, verts, params, st = build(
        ...
        max_cells=400000, cyl_cap=6, shell_factor=sf, curv_c=CURV_C,
        proj_tol=0.02 * tf)
    ...
    rec = {"shell_factor": sf, "tol_factor": tf, "nodes": len(verts),
           "x": ax["x"], "y": ax["y"], "z": ax["z"],
           "dx_pct": round(dx * 100, 2), "dy_pct": round(dy * 100, 2),
           "score": abs(dx) + abs(dy),
           "median": m["oracle_to_our"][1],
           "c1e6": m["oracle_matched_1e-6"], "c1e4": m["oracle_matched_1e-4"],
...
def main():
    sfs = (0.6, 0.8, 1.05, 1.4)
    tfs = (0.15, 0.3, 0.45, 0.6)
    out = []
    for sf in sfs:
        for tf in tfs:
            try:
                ...
                print({"shell_factor": sf, "tol_factor": tf,
                       "error": repr(e)}, flush=True)
    out.sort(key=lambda r: r.get("score", 9e9))
    pout = os.path.join(ROOT, "tools", "probe_work", "hdm_sweep2d.json")
