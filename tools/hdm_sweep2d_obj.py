# -*- coding: utf-8 -*-
"""P18e: 2D sweep with object size field ON (use_object_sizes=True) —
push y deviation into +-3% while keeping x close."""
import json
import os
import sys
import time
...
def run(sf, tf, max_cells=400000):
    jdir = os.path.join(r"D:\training\icepak", JOB)
    t0 = time.time()
...
def main():
    ...
    pout = os.path.join(ROOT, "tools", "probe_work", "hdm_sweep2d_obj.json")
