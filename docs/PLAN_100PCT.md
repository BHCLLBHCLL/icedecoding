...
- **J1b 多锚点/变体拟合器**：x[0.12,0.18] 出现 g0=grid_size_h/2 变体与不对称链；过渡带 92 簇层的 z 链、y 向多锚段——建规则空间搜索拟合器，目标：150 层 z 全清单 + 粗区 38×45 全线集由定律重生成（1e-12）。
- **J1c 主层 2D 细化 pattern**：3,212 节点主层的圆周细化逐层重建（等计数定律 + 圆-四叉树相交规则）；簇层圆周 pattern。
- **J2 层栈规则/平衡判据**：z 层选择与细化的关联（哪些层获得哪级细化）、层间 hanging 对应。
- **J3 节点集装配与逐点验收**：层 × 2D 网格 → 全节点集，与 oracle 1e-9 逐点比对；节点数恒等验收。
