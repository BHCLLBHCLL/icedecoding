...
| **P19-6** ECAD 收官 | **大部分(~85%)** | ECXML/IDF/IDX/Networks/JEDEC/powermap/ICB 解析 + export_idf/network/jedec；icb_metal_fractions；AEdt 脚本导出；**ODB++/ANF→ICB oracle 管线**（iceecad 广义 mode Table + GUI 导入）；**Show metal fractions 视区显示**（ice_view3d.metal_fraction_actors 逐层铜箔 + 图例） | Export AE 脚本等剩余导出格式 |
