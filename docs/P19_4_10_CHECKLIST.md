...
| 项 | 状态 | 已实现 | **未实现（缺口）** |
| --- | --- | --- | --- |
| **P19-4** 求解/后处理/报告 | **部分(~65%)** | fdat 真实数据源；cas 面重建 cell→node（58,908 单元）；真实 VTK 温度云（12-1:124k 点）+ **iso/平面切/极值云按真实温着色**（蓝→红）；矢量 glyph（124k 箭头）；**真插值等值面**（`iso_surface_polys` KD 网格填充 + vtkContourFilter 三角面）；**History 曲线真实瞬态监测点**；PlotWindow 温度直方图；报告 HTML 温度统计段+SVG | ② 其余曲线真实化（Network temperature / 3D Variation 仍合成分支；convergence 无 .resd 时回退模拟）；③ 瞬态设置/单位/zoom-in 模型/powermap 显示；④ Solve 面板字段全集（Basic/Advanced/Parallel/Patch/trials/ROM）；⑤ 报告全套（Summary/Point/Full/Overview/网络块值/Fan 工作点/EM/Solar/Autotherm/5 导出）——ice_report 仅 html_report+summary_data |
