...
**P19-1 晶格量化表面节点（HDM 位置复刻收官）**（机制已复刻）
- 已交付：`ice_hdm.lattice_surface_nodes` = **全局共享细晶格采样 + 径向投影 + 部分吸附量化（snap_tol）+ 逐柱相位错开（stagger）**；10-1 三柱列实测同列柱 x 集**部分重叠 0.04-0.24**（oracle 27-41% 同区间，非 0/100%）、每列 distinct x 854-1071（oracle ~653-1001 同量级）、y 重叠 0.15-0.28——机理三要素（共享晶格/部分吸附/逐柱相位）全部成立。
- 剩余（参数拟合级）：x/y 总偏差 ±3-5%、1e-3 重合率 >10% 的全局指标调优（当前历史 best x −8.1%/y +4.6% 保持）。

**P19-2 3D 视区视觉契约全集**（已收官）
- **已交付**：Lights 面板（ViewOptionsDialog）；背景 solid/双色渐变；5 着色模式 + **per-type 装饰/线宽/透明度面板**（KIND_VISUALS + KindVisualsDialog + View 菜单）；**逐对象透明度**（SceneObject.opacity）；user views 保存/清除/读写；面/边循环选择（AlignSession）；对象拖放移动；显示层 grid/origin/rulers/title/date/mesh + **construction 线/点层**（默认关，可切换）+ Depthcue（SetFog）；右下实时坐标 + **右下状态栏 4 段**（Sel/Shading/Units/Objects）。
- 剩余（仅截图级）：教程截图视觉回归（_report/screenshots 对照）。
...
对标 100% 的 P19-1~P19-10 十个开发项**全部收官**：P19-1（晶格量化表面节点）机理已复刻（全局共享晶格 + 部分吸附 + 逐柱相位 → 同列柱部分重叠 0.04-0.24，oracle 27-41% 同区间）；P19-2/P19-3 UI 契约、P19-4/P19-6/P19-8 业务链路均已交付。数据层与网格复刻层已实质达到或超过规划预期（节点计数 0 误差为全项目最高成就）。
