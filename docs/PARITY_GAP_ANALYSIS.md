...
| ECAD | ice_ecad | ECXML/IDF/IDX/Networks/JEDEC/powermap 5 格式/ICB 解析/EM mapping/metal fractions | D3 | ODB++/ANF→ICB oracle 管线；AEdt 脚本导出；5 导出格式收尾 |
...
6. **ECAD 收官**：ODB++/ANF→ICB oracle 管线、AEdt 导出、metal fractions 显示。
...
**P19-6 ECAD 收官**
- ODB++/ANF→ICB oracle 管线（iceecad.exe 沙箱）；AEdt 脚本导出；Show metal fractions 显示；5 导出格式收尾。
