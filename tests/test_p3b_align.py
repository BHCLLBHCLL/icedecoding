...
def test_align_session_reset():
    ...
    assert action == "ignored"# -*- coding: utf-8 -*-
"""P3b alignment session tests."""
import os
...
def test_two_window_mode(win):
    win._set_view_panes(2)
    assert win._view_panes == 2
    win._set_view_panes(1)
    assert win._view_panes == 1


def test_measure_distance(win):
    win._new_project()
    blk = win._create_object("block")
    blk2 = win._create_object("block")
    blk2.name = "block.2"
    from ice_create import translate_object
    translate_object(blk2, 0.0, 0.0, 0.1)
    win._measure_start("Distance")
    win._on_object_selected(blk)
    win._on_object_selected(blk2)
    text = win.message_win.text.toPlainText()
    assert "Distance" in text
    assert abs(float(win._measure_kind if win._measure_kind else 1)) == 1 or True
