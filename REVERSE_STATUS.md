...
- **主层构成（tools/hdm_j1c_refine.py）**：7 个柱跨层每层 3,212 = **1,172 粗节点保留 + 2,040 细化新增 − 538 粗节点被替换**（层网格独立重生成，非增量细化；z=0.19 微差 2,038/536/1,174）。**7 层细化集两两交集恒为 760**——层无关固定图案 + 每层特有部分（2,040−760 = 1,280）。
- **24 角星形环带（tools/probe_work/j1c_ring.json，三层逐位相同）**：每柱每层在 ρ = r(z) 处恰 **24 个环节点**，角度集固定 {0, ±17.641°, ±34.919°, ±45°, ±55.081°, ±72.359°, ±90°}（关于 45° 自对称），坐标对/r = (±1,0)、(±0.952976, ±0.303046)、(±0.819961, ±0.57242)、(±0.707107, ±0.707107) 及镜像——**随 r(z) 精确缩放**，即环 = 固定 24 角星 × r(z)。环坐标对满足勾股闭合（dx²+dy²=r² 且 dx、dy 同属偏移集）→ 环点可由 r(z) 精确再生；角度集的推导（为何是 17.641/34.919）为遗留项。
- **固定 8 重数半径**：ρ ∈ {0.006632, 0.008509, 0.009171, 0.013794, 0.014366…}（跨层不变的绝对值，非 r 缩放）×8 节点/层——层无关图案的一部分（760 共享）；另有 94 个远离柱的细化节点（x≈0.105/0.295、y≈0.205/0.395 = block/quad 面邻域）7 层恒定。
- **柱内节点**：每柱 73 个 ρ<r(z) 的节点（solid 内部有网格节点），含微小固定偏移（±0.000119/0.000358/0.000722/0.001074/0.00125=H/4）。
- **逐柱计数不等**（192-288）：柱的局部骨架线环境不同（各列/行邻近骨架线集不同），细化规模随环境变化。
- **下一步**：①分离每柱细化节点为「骨架环境 + 24 角星 + 固定图案 + 柱内」四类并逐类求生成律；②角度集 17.641°/34.919° 的推导；③5738 被替换粗节点的空间归属。
