...
- **四总体分离（tools/probe_work/j1c_populations.json，7 层严格恒定）**：每层细化 2,040 = **fixed 730（层无关柱固定模板）+ far 30（block/quad 面邻域）+ ring 222（24 角星 ×9 柱，逐柱 22-28）+ other 1,058**（z=0.19: other 1,054 + on_skel 2）。T-A 证实：共享 760 = 730 柱固定 + 30 far。
- **24 角星形环带（tools/probe_work/j1c_ring.json，三层逐位相同）**：每柱每层在 ρ = r(z) 处环带角度集固定 {0, ±17.641°, ±34.919°, ±45°, ±55.081°, ±72.359°, ±90°}（关于 45° 自对称），坐标对/r = (±1,0)、(±0.952976, ±0.303046)、(±0.819961, ±0.57242)、(±0.707107, ±0.707107) 及镜像——**随 r(z) 精确缩放**；弧长间隙回文 0.3079/0.3016/0.1759 rad。环点满足勾股闭合（dx²+dy²=r² 且 dx、dy 同属偏移集）。角度集推导为遗留项。
- **柱 1 固定模板 93 偏移（9 位精度入档）**：细晶格族 {0, ±0.002909, ±0.005960, ±0.006113, ±0.007997, ±0.008228}² 组合（0.007997≈r_mid/2、0.002909≈2/11×r_mid，r_mid=0.016=两底半径均值——模板以中值半径为尺度）、巨大偏移曲线族（|dx|≈0.029-0.046 配骨架 dy——被位移的骨架线，见 other 类）。
- **other 类 = 被位移的骨架线（T-D 联动）**：538 个被替换粗节点 100% 在 block 区内、距柱轴 0.02-0.06；例：骨架列 x=0.105 在柱邻域被曲线 x≈0.104577（随 dy 微弯）替代——other 1,058 即这些位移线的交点网。位移律为遗留项。
- **柱内节点**：每柱 73 个 ρ<r(z) 节点（solid 内部有网格），含微小固定偏移（±0.000119/0.000358/0.000722/0.001074/0.00125=H/4）。
- **下一步**：①other 类位移线定律（0.104577 vs 0.105，/2400 单位差 1）；②24 角星角度推导；③fixed 模板与 r_mid 的精确关系（0.002909=2r_mid/11?）；④z=0.19 的 on_skel=2 微差。
